...
class TianyanAIClient:
    ...
    def ensure_available(self) -> str:
        ...
        if not path:
            raise TianyanAIError(
                "Tianyan AI CLI command `tyc` was not found. Install tyc-cli and run `tyc init` first."
            )
        return path

    def run_json(self, args: list[str]) -> dict[str, Any]:
        """Run a `tyc` command and parse JSON output."""
...
def _redact_secret(text: str) -> str:
    # Tianyan keys currently look like mcpk2_..., but keep the pattern broad.
    import re
