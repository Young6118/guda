...
import { runTask, getTaskDetail } from './api.js'
...
async function rerunTask(task) { try { await runTask(task.id); await refresh(); ElMessage.success('任务运行完成') } catch (error) { ElMessage.error(error?.response?.data?.detail || error.message || '任务运行失败'); await refresh() } }
