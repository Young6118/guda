...
import { runTask, getTaskDetail, updateTaskSchedule } from './api.js'
...
async function showTask(task) { try { selectedTask.value = (await getTaskDetail(task.id)).data; taskDrawer.value = true } catch (error) { ElMessage.error(error?.response?.data?.detail || error.message) } }
async function saveSchedule(task) { try { await updateTaskSchedule(task.id, { schedule: task.schedule || null, enabled: Boolean(task.enabled) }); await refresh(); ElMessage.success(task.schedule ? `已设置每 ${task.schedule} 自动运行` : '已关闭自动调度') } catch (error) { ElMessage.error(error?.response?.data?.detail || error.message || '调度设置失败') } }
function openEvidence(row) { selected.value = row; drawer.value = true }
function selectWord(word) { store.filters.q = word; refresh() }
...
<template>
  ...
  <el-drawer v-model="taskDrawer" :title="selectedTask?.task?.name || '任务详情'" size="min(760px, 94vw)"><div v-if="selectedTask"><el-descriptions :column="2" border><el-descriptions-item label="主题包">{{ selectedTask.task.topic_pack_name }}</el-descriptions-item><el-descriptions-item label="查询">{{ selectedTask.task.query }}</el-descriptions-item><el-descriptions-item label="最大抓取量">{{ selectedTask.task.max_items_per_run }}</el-descriptions-item><el-descriptions-item label="运行记录">{{ selectedTask.runs.length }} 次</el-descriptions-item></el-descriptions><h3>运行历史</h3><el-table :data="selectedTask.runs" stripe size="small"><el-table-column prop="started_at" label="开始时间" min-width="180" /><el-table-column prop="status" label="状态" width="100" /><el-table-column prop="items_fetched" label="抓取" width="70" /><el-table-column prop="items_normalized" label="规范化" width="80" /><el-table-column prop="items_deduped" label="去重" width="70" /><el-table-column prop="error_summary" label="错误" min-width="180" show-overflow-tooltip /></el-table></div></el-drawer>
