...
<template>
  <el-container class="shell">
    ...
    <el-main class="main">
      ...
      <el-card shadow="never" class="filter-card"><el-form inline @submit.prevent="refresh"><el-form-item label="主题包"><el-select v-model="store.topicPackId" filterable style="width:260px"><el-option v-for="item in store.topicPacks" :key="item.id" :label="`${item.name} · ${item.evidence_count || 0} 条证据`" :value="item.id" /></el-select></el-form-item><el-form-item label="关键词"><el-input v-model="store.filters.q" placeholder="搜索证据、标题或平台" clearable @keyup.enter="refresh" /></el-form-item><el-form-item><el-button type="primary" @click="refresh">应用筛选</el-button></el-form-item></el-form></el-card>
