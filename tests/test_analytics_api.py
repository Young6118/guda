...
def test_app_analytics_returns_word_cloud_and_distributions(tmp_path: Path) -> None:
    ...
    raw_id = repo.insert_raw_item(source_id=source_id, task_id=task_id, run_id=run_id, platform_item_id="1", url="https://example.com/1", metadata=__import__("guda.raw_store", fromlist=["RawMetadata"]).RawMetadata(raw_uri="raw/1.json", raw_sha256="sha", raw_size_bytes=1), acquisition_layer="official_api", fetched_at="2026-07-31T00:00:00Z", raw_content_hash="hash")
    ...
    res = client.get("/api/app/analytics", params={"topic_pack_id": topic_id}, auth=("admin", "secret"))

    assert res.status_code == 200
    body = res.json()
    assert body["summary"]["evidence_count"] == 1
    assert body["platforms"] == [{"name": "github", "value": 1}]
    assert any(item["name"] == "deployment" for item in body["word_cloud"])
    assert body["topics"] == [{"name": "deployment", "value": 1}]
