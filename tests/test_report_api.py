...
def test_app_report_contains_insights_and_evidence_citations(tmp_path: Path) -> None:
    ...
    res = client.get("/api/app/report", params={"topic_pack_id": topic_id, "days": 30}, auth=("admin", "secret"))

    assert res.status_code == 200
    body = res.json()
    assert body["title"] == "AI Watch 分析报告"
    assert "## 关键洞察" in body["markdown"]
    assert "https://example.com/1" in body["markdown"]
